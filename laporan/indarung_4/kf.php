...
<body>
    ...
    $arah_EP_e = isset($_GET['arah_EP_e']) && $_GET['arah_EP_e'] !== '' ? $_GET['arah_EP_e'] : NULL;
    ...
    $sql = "UPDATE kf_4 SET JAM=?, kf90u=?, kf180u=?, H2O=?, SiO2=?, Al2O3=?, Fe2O3=?, CaO=?, MgO=?, SO3=?, K2O=?, Na2O=?, Cl2=?, arah_EP=?, LEVEL_cf=?, FEED_kiln=?, FEED_fc=? WHERE id=?";
    ...
    mysqli_stmt_bind_param($stmt, "sssssssssssssssssi", $JAM_e, $kf90u_e, $kf180u_e, $H2O_e, $SiO2_e, $Al2O3_e, $Fe2O3_e, $CaO_e, $MgO_e, $SO3_e, $K2O_e, $Na2O_e, $Cl2_e, $arah_EP_e, $LEVEL_cf_e, $FEED_kiln_e, $FEED_fc_e, $id_e);
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM kf_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM kf_4 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
 else {
    $sql_filtered = "SELECT * FROM kf_4 WHERE TANGGAL = CURDATE()";
}
...
?>
		...
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>KILN FEED Indarung 4</label>
		</center>
	...
	<table class="adminlist mb-3" border="1" width="500" cellpadding="5" >
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
				       $count_query = mysqli_query($conn, "SELECT COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(kf90u) AS count_kf90u, COUNT(kf180u) AS count_kf180u, COUNT(H2O) AS count_h2o, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(Cl2) AS count_cl2, COUNT(SUM) AS count_sum, COUNT(ALKALI) AS count_alkali FROM kf_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM kf_4");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LSF) AS max_lsf, MAX(SIM) AS max_sim, MAX(ALM) AS max_alm,MAX(kf90u) as max_kf90u,MAX(kf180u) as max_kf180u, MAX(H2O) AS max_h2o, MAX(SiO2) AS max_sio2, MAX(Al2O3) AS max_al2o3, MAX(Fe2O3) AS max_fe2o3, MAX(CaO) AS max_cao, MAX(MgO) AS max_mgo, MAX(SO3) AS max_so3, MAX(K2O) AS max_k2o, MAX(Na2O) AS max_na2o, MAX(Cl2) AS max_cl2, MAX(SUM) AS max_sum, MAX(ALKALI) AS max_alkali FROM kf_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <table class="table table-striped table-bordered table-hover m">
				<thead>
					...
					<tr class="bg-primary text-white">
						...
						<th>Tanggal</th><th>Jam</th><th>LSF</th> <th>SIM</th> <th>ALM</th><th>BF</th><th>90u</th><th>180u</th> <th>H2O</th><th>SiO2</th> <th>Al2O3</th> <th>Fe2O3</th> <th>CaO</th> <th>MgO</th> <th>SO3</th> <th>K2O</th> <th>Na2O</th> <th>Cl2</th><th>SUM</th><th>Alkali</th><th>ARAH EP</th> <th>LEVEL CF SILO</th> <th>FEED KILN</th> <th>FEED FC</th><th>TON EMISI CO2</th><th>Report</th>
				...
				<tbody>
					    ...
					    while ($d = mysqli_fetch_array($q)) {
					        ...
					        $update_query = "UPDATE kf_4 SET LSF=?, SIM=?, ALM=?, SUM=?, ALKALI=?, BF=?, TON_emisi=? WHERE id=?";
					        ...
					        $w_180u = '';
					        if ($d['kf180u'] > $configs['kf180u4']['max'] ) {
					            $w_180u = 'text-danger';
					        }
					        $w_90u = '';
							if ($d['kf90u'] > $configs['kf90u4']['max'] ) {
							    $w_90u = 'text-danger';
							}
							$w_h2o = '';
							if ($d['H2O'] < $configs['H2Okf4']['min'] || $d['H2O'] > $configs['H2Okf4']['max']) {
							    $w_h2o = 'text-danger';
							}
							$w_alm = '';
							if ($alm < $configs['ALMkf4']['min'] || $alm > $configs['ALMkf4']['max']) {
							    $w_alm = 'text-danger';
							}
					        $w_sim = '';
							if ($sim < $configs['SIMkf4']['min'] || $sim > $configs['SIMkf4']['max']) {
							    $w_sim = 'text-danger';
							}
							$w_lsf = '';
							if ($lsf < $configs['LSFkf4']['min'] || $lsf > $configs['LSFkf4']['max']) {
							    $w_lsf = 'text-danger';
							}
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM kf_4 where id ='$id' ";
											...
											?>
												...
												<div class="form-group row my-0">
												    <label for="arah_EP" class="col-sm-4 col-form-label">Arah EP</label>
												    <div class="col-sm-8">
												        <select name="arah_EP_e" id="arah_EP_e" style="height:30px" class="form-control">
												            <option value=""></option> 
												            <option value="SILO" <?= ($row['arah_EP'] == 'SILO') ? 'selected' : '' ?>>SILO</option>
												            <option value="KILN" <?= ($row['arah_EP'] == 'KILN') ? 'selected' : '' ?>>KILN</option>
												        </select>
												    </div>
												</div>
												<div class="form-group row my-0">
	...
	$arah_EP = !empty($_POST['arah_EP']) ? $_POST['arah_EP'] : NULL;
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					if (isset($_POST['submit'])) {
					    ...
					    $sql = "INSERT INTO kf_4 (TANGGAL, JAM, kf90u, kf180u, H2O, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, Cl2, arah_EP, LEVEL_cf, FEED_kiln, FEED_fc, waktu, iduser) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

					    $stmt = mysqli_prepare($conn, $sql);

					    if ($stmt) {

					        mysqli_stmt_bind_param($stmt, "ssssssssssssssssssss", $tanggal, $JAM, $kf90u, $kf180u, $H2O, $SiO2, $Al2O3, $Fe2O3, $CaO, $MgO, $SO3, $K2O, $Na2O, $Cl2, $arah_EP, $LEVEL_cf, $FEED_kiln, $FEED_fc, $waktu, $username);
					...
					$sql = "SELECT * FROM kf_4 ORDER BY TANGGAL DESC";
					$q = mysqli_query($conn, $sql);
					?>
					...
					<div class="container">
						<form method="POST" onsubmit="return validateForm()">
							...
							<div class="form-group row my-0">
							    <label for="arah_EP" class="col-sm-4 col-form-label">ARAH EP</label>
							    <div class="col-sm-8">
							        <select name="arah_EP" id="arah_EP" style="height:30px" class="form-control">
			                            <option value="" ></option>
