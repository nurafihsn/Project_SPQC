...
<body>
    ...
    $fields = ['JAM', 'rm90u', 'rm180u', 'H2O', 'LS_act', 'SS_act', 'CL_act', 'IS_act', 'TON_dry', 'LS_qcx', 'SS_qcx', 'CL_qcx', 'IS_qcx', 'LS_set', 'SS_set', 'CL_set', 'IS_set', 'SiO2', 'Al2O3', 'Fe2O3', 'CaO', 'MgO', 'SO3', 'K2O', 'Na2O', 'Cl2', 'LSToPZ'];
    ...
    if ($id_e) {
        ...
        $sql = "UPDATE r2_4 SET $setClause WHERE id = ?";
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM r2_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM r2_4 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
else {
    $today = date("Y-m-d");
    $sql_filtered = "SELECT * FROM r2_4 WHERE TANGGAL = '$today'";
}
...
?>
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>RAW MIX 2 Indarung 4</label>
		</center>
	...
	<table class="adminlist mb-3 " border="1" width="500" cellpadding="5"  >
		<thead>
					...
					<tr class="text-center" >
			            <th rowspan="1"></th><th  colspan="6">4R2</th><th  colspan="4">Proposi Actual</th><th colspan="1">TON</th><th colspan="4">Setpoint QCX, %Wet</th><th colspan="4">Proporsi SET POINT</th> <th colspan="11">OKSIDA</th><th colspan="1">TON</th>
			        </tr>
				...
				<tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				       ...
				       $count_query = mysqli_query($conn, "SELECT COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(rm90u) AS count_rm90u, COUNT(rm180u) AS count_rm180u, COUNT(H2O) AS count_h2o, COUNT(LS_act) AS count_ls_act, COUNT(SS_act) AS count_ss_act, COUNT(CL_act) AS count_cl_act, COUNT(IS_act) AS count_is_act, COUNT(TON_dry) AS count_ton_dry, COUNT(LS_qcx) AS count_ls_qcx, COUNT(SS_qcx) AS count_ss_qcx, COUNT(CL_qcx) AS count_cl_qcx, COUNT(IS_qcx) AS count_is_qcx, COUNT(LS_set) AS count_ls_set, COUNT(SS_set) AS count_ss_set, COUNT(CL_set) AS count_cl_set, COUNT(IS_set) AS count_is_set, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(Cl2) AS count_cl2, COUNT(SUM) AS count_sum, COUNT(ALKALI) AS count_alkali, COUNT(TON_emisi) AS count_ton_emisi  FROM r2_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM r2_4");
			...
			<tr bgcolor="YELLOW" style="color:BLACK">
				...
				if (isset($_GET['awal']) && isset($_GET['akhir'])) {
				    ...
				    $datain_query = mysqli_query($conn, "SELECT 
				        ...
				        FROM r2_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
		...
		<div>
			 <table class="table table-striped table-bordered table-hover">
				<thead>
					...
					<tr class="text-center" >
			            <th colspan="2"></th><th  colspan="6">4R2</th><th  colspan="4">Proposi Actual</th><th colspan="1">TON</th><th colspan="4">Setpoint QCX, %Wet</th><th colspan="4">Proporsi SET POINT</th> <th colspan="12">OKSIDA</th><th colspan="1">TON</th><th colspan="4">LS BACK</th>
			        </tr>
				...
				<tbody>
					    ...
					    while ($d = mysqli_fetch_array($q)) {
					       ...
					       $update_query = "UPDATE r2_4 SET LSF=?, SIM=?, ALM=?,IS_act=?,IS_qcx=?,IS_set=?, SUM=?, ALKALI=?, TON_emisi=?, SiO2_back=?, Al2O3_back=?, Fe2O3_back=?, CaO_back=? WHERE id=?";
						...
						mysqli_stmt_close($stmt);
							...
							$w_180u = '';
					        if ($d['rm180u'] > $configs['rm180u4c']['max'] ) {
					            $w_180u = 'text-danger';
					        }
					        $w_90u = '';
					        if ($d['rm90u'] > $configs['rm90u4c']['max'] ) {
					            $w_90u = 'text-danger';
					        }
					        $w_alkali  = '';
							if ($alkali > 0.35) {
					        ...
					        $w_h2o = '';
							if ($d['H2O'] < $configs['H2Orm4c']['min'] || $d['H2O'] > $configs['H2Orm4c']['max']) {
							    $w_h2o = 'text-danger';
							}
							$w_alm = '';
							if ($alm < $configs['ALMrm4c']['min'] || $alm > $configs['ALMrm4c']['max']) {
							    $w_alm = 'text-danger';
							}
					        $w_lsf = '';
							if ($lsf < $configs['LSFrm4c']['min'] || $lsf > $configs['LSFrm4c']['max']) {
							    $w_lsf = 'text-danger';
							}
							$w_sim = '';
							if ($sim < $configs['SIMrm4c']['min'] || $sim > $configs['SIMrm4c']['max']) {
							    $w_sim = 'text-danger';
							}
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM r2_4 where id ='$id' order by id";
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					if (isset($_POST['submit'])) {
					    $waktu = date("H:i:s");
					    $tanggal = date("Y-m-d");
					    $sql = "INSERT INTO r2_4 (TANGGAL, JAM, rm90u, rm180u, H2O, LS_act, SS_act, CL_act, TON_dry, LS_qcx, SS_qcx, CL_qcx, LS_set, SS_set, CL_set, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, Cl2, LSToPZ, waktu, iduser) 
					            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
					...
					$sql = "SELECT * FROM r2_4 WHERE TANGGAL = CURDATE()";
					$q = mysqli_query($conn, $sql);
					?>
