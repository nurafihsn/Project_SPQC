...
<body>
    ...
    $fields = [
        'SiO2_b', 'Al2O3_b', 'Fe2O3_b', 'CaO_b', 'MgO_b', 'SO3_b', 'K2O_b', 'Na2O_b', 'Cl2_b',
        'SiO2_c', 'Al2O3_c', 'Fe2O3_c', 'CaO_c', 'MgO_c', 'SO3_c', 'K2O_c', 'Na2O_c', 'Cl2_c',
        'RM1', 'RM2'
    ];
    ...
    $sql = "UPDATE ep_4 SET " . implode(", ", $placeholders) . " WHERE id=?";
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM ep_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM ep_4 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
 else {
    $sql_filtered = "SELECT * FROM ep_4 WHERE TANGGAL = CURDATE()";
}
...
?>
	...
	<table class="adminlist mb-3 " border="1" width="500" cellpadding="5"  >
					<tr class="text-center" >
			            <th rowspan="1"></th><th colspan="14">EP 3B</th><th  colspan="14">EP 3C</th>
			        </tr>
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			      $count_query = mysqli_query($conn, "SELECT COUNT(LSF_b) AS count_lsfb, COUNT(SIM_b) AS count_simb, COUNT(ALM_b) AS count_almb, COUNT(SiO2_b) AS count_sio2b, COUNT(Al2O3_b) AS count_al2o3b, COUNT(Fe2O3_b) AS count_fe2o3b, COUNT(CaO_b) AS count_caob, count(MgO_b) AS count_mgob, COUNT(SO3_b) AS count_so3b, COUNT(K2O_b) AS count_k2ob, COUNT(Na2O_b) AS count_na2ob, COUNT(Cl2_b) AS count_cl2b, COUNT(SUM_b) AS count_sumb, COUNT(ALKALI_b) AS count_alkalib, COUNT(LSF_c) AS count_lsfc, COUNT(SIM_c) AS count_simc, COUNT(ALM_c) AS count_almc, COUNT(SiO2_c) AS count_sio2c, COUNT(Al2O3_c) AS count_al2o3c, COUNT(Fe2O3_c) AS count_fe2o3c, COUNT(CaO_c) AS count_caoc, COUNT(MgO_c) AS count_mgoc, COUNT(SO3_c) AS count_so3c, COUNT(K2O_c) AS count_k2oc, COUNT(Na2O_c) AS count_na2oc, COUNT(Cl2_c) AS count_cl2c, COUNT(SUM_c) AS count_sumc, COUNT(ALKALI_c) AS count_alkalic FROM ep_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			       ...
			       $count_result = mysqli_fetch_assoc($count_query);
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM ep_4");
			      ...
			      <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			      $min_query = mysqli_query($conn, "SELECT MIN(LSF_b) AS min_lsfb, MIN(SIM_b) AS min_simb, MIN(ALM_b) AS min_almb, MIN(SiO2_b) AS min_sio2b, MIN(Al2O3_b) AS min_al2o3b, MIN(Fe2O3_b) AS min_fe2o3b, MIN(CaO_b) AS min_caob, MIN(MgO_b) AS min_mgob, MIN(SO3_b) AS min_so3b, MIN(K2O_b) AS min_k2ob, MIN(Na2O_b) AS min_na2ob, MIN(Cl2_b) AS min_cl2b, MIN(SUM_b) AS min_sumb, MIN(ALKALI_b) AS min_alkalib, MIN(LSF_c) AS min_lsfc, MIN(SIM_c) AS min_simc, MIN(ALM_c) AS min_almc, MIN(SiO2_c) AS min_sio2c, MIN(Al2O3_c) AS min_al2o3c, MIN(Fe2O3_c) AS min_fe2o3c, MIN(CaO_c) AS min_caoc, MIN(MgO_c) AS min_mgoc, MIN(SO3_c) AS min_so3c, MIN(K2O_c) AS min_k2oc, MIN(Na2O_c) AS min_na2oc, MIN(Cl2_c) AS min_cl2c, MIN(SUM_c) AS min_sumc, MIN(ALKALI_c) AS min_alkalic FROM ep_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			       $average_query = mysqli_query($conn, "SELECT AVG(LSF_b) AS avg_lsfb, AVG(SIM_b) AS avg_simb, AVG(ALM_b) AS avg_almb, AVG(SiO2_b) AS avg_sio2b, AVG(Al2O3_b) AS avg_al2o3b, AVG(Fe2O3_b) AS avg_fe2o3b, AVG(CaO_b) AS avg_caob, AVG(MgO_b) AS avg_mgob, AVG(SO3_b) AS avg_so3b, AVG(K2O_b) AS avg_k2ob, AVG(Na2O_b) AS avg_na2ob, AVG(Cl2_b) AS avg_cl2b, AVG(SUM_b) AS avg_sumb, AVG(ALKALI_b) AS avg_alkalib, AVG(LSF_c) AS avg_lsfc, AVG(SIM_c) AS avg_simc, AVG(ALM_c) AS avg_almc, AVG(SiO2_c) AS avg_sio2c, AVG(Al2O3_c) AS avg_al2o3c, AVG(Fe2O3_c) AS avg_fe2o3c, AVG(CaO_c) AS avg_caoc, AVG(MgO_c) AS avg_mgoc, AVG(SO3_c) AS avg_so3c, AVG(K2O_c) AS avg_k2oc, AVG(Na2O_c) AS avg_na2oc, AVG(Cl2_c) AS avg_cl2c, AVG(SUM_c) AS avg_sumc, AVG(ALKALI_c) AS avg_alkalic FROM ep_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LSF_b) AS max_lsfb, MAX(SIM_b) AS max_simb, MAX(ALM_b) AS max_almb, MAX(SiO2_b) AS max_sio2b, MAX(Al2O3_b) AS max_al2o3b, MAX(Fe2O3_b) AS max_fe2o3b, MAX(CaO_b) AS max_caob, MAX(MgO_b) AS max_mgob, MAX(SO3_b) AS max_so3b, MAX(K2O_b) AS max_k2ob, MAX(Na2O_b) AS max_na2ob, MAX(Cl2_b) AS max_cl2b, MAX(SUM_b) AS max_sumb, MAX(ALKALI_b) AS max_alkalib, MAX(LSF_c) AS max_lsfc, MAX(SIM_c) AS max_simc, MAX(ALM_c) AS max_almc, MAX(SiO2_c) AS max_sio2c, MAX(Al2O3_c) AS max_al2o3c, MAX(Fe2O3_c) AS max_fe2o3c, MAX(CaO_c) AS max_caoc, MAX(MgO_c) AS max_mgoc, MAX(SO3_c) AS max_so3c, MAX(K2O_c) AS max_k2oc, MAX(Na2O_c) AS max_na2oc, MAX(Cl2_c) AS max_cl2c, MAX(SUM_c) AS max_sumc, MAX(ALKALI_c) AS max_alkalic FROM ep_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $sd_query = mysqli_query($conn, "SELECT STDDEV(LSF_b) AS sd_lsfb, STDDEV(SIM_b) AS sd_simb, STDDEV(ALM_b) AS sd_almb, STDDEV(SiO2_b) AS sd_sio2b, STDDEV(Al2O3_b) AS sd_al2o3b, STDDEV(Fe2O3_b) AS sd_fe2o3b, STDDEV(CaO_b) AS sd_caob, STDDEV(MgO_b) AS sd_mgob, STDDEV(SO3_b) AS sd_so3b, STDDEV(K2O_b) AS sd_k2ob, STDDEV(Na2O_b) AS sd_na2ob, STDDEV(Cl2_b) AS sd_cl2b, STDDEV(SUM_b) AS sd_sumb, STDDEV(ALKALI_b) AS sd_alkalib, STDDEV(LSF_c) AS sd_lsfc, STDDEV(SIM_c) AS sd_simc, STDDEV(ALM_c) AS sd_almc, STDDEV(SiO2_c) AS sd_sio2c, STDDEV(Al2O3_c) AS sd_al2o3c, STDDEV(Fe2O3_c) AS sd_fe2o3c, STDDEV(CaO_c) AS sd_caoc, STDDEV(MgO_c) AS sd_mgoc, STDDEV(SO3_c) AS sd_so3c, STDDEV(K2O_c) AS sd_k2oc, STDDEV(Na2O_c) AS sd_na2oc, STDDEV(Cl2_c) AS sd_cl2c, STDDEV(SUM_c) AS sd_sumc, STDDEV(ALKALI_c) AS sd_alkalic FROM ep_4 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <table class="table table-striped table-bordered table-hover">
				...
				<tbody>
					    ...
					    while ($d = mysqli_fetch_array($q)) {
					      ...
					      $update_query = "UPDATE ep_4 SET LSF_b=?, SIM_b=?, ALM_b=?, SUM_b=?, ALKALI_b=?, LSF_c=?, SIM_c=?, ALM_c=?, SUM_c=?, ALKALI_c=? WHERE id=?";
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM ep_4 where id ='$id' order by id";
											...
											?>
												...
												<div class="form-group row my-0">
												    <label for="Cl2_b" class="col-sm-4 col-form-label">Cl2</label>
												    <div class="col-sm-8">
												        <input type="number_format" style="height:30px" class="form-control" id="Cl2_b_e" name="Cl2_b_e" value="<?= $row['Cl2_b']; ?>">
												    </div>
												</div>
												...
												<div class="form-group row my-0">
												    <label for="Cl2_c" class="col-sm-4 col-form-label">Cl2</label>
												    <div class="col-sm-8">
												        <input type="number_format" style="height:30px" class="form-control" id="Cl2_c_e" name="Cl2_c_e" value="<?= $row['Cl2_c']; ?>">
												    </div>
												</div>
	...
	$Cl2_b = !empty($_POST['Cl2_b']) ? $_POST['Cl2_b'] : NULL;
	...
	$Cl2_c = !empty($_POST['Cl2_c']) ? $_POST['Cl2_c'] : NULL;
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					if (isset($_POST['submit'])) {
						...
						$sql = "INSERT ep_4 (TANGGAL,SiO2_b, Al2O3_b, Fe2O3_b, CaO_b, MgO_b, SO3_b, K2O_b, Na2O_b, Cl2_b,SiO2_c, Al2O3_c, Fe2O3_c, CaO_c, MgO_c, SO3_c, K2O_c, Na2O_c, Cl2_c,  RM1, RM2, waktu, iduser) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
						$stmt = mysqli_prepare($conn, $sql);

						    // Bind parameter ke statement
						    mysqli_stmt_bind_param($stmt, "sssssssssssssssssssssss", $tanggal, $SiO2_b, $Al2O3_b, $Fe2O3_b, $CaO_b, $MgO_b, $SO3_b, $K2O_b, $Na2O_b, $Cl2_b,$SiO2_c, $Al2O3_c, $Fe2O3_c, $CaO_c, $MgO_c, $SO3_c, $K2O_c, $Na2O_c, $Cl2_c, $RM1 ,$RM2 , $waktu, $username);
					...
					<div class="container">
						<form method="POST" onsubmit="return validateForm()">
							...
							<div class="form-group row my-0">
							    <label for="Cl2_b" class="col-sm-4 col-form-label">Cl2</label>
							    <div class="col-sm-8">
							        <input type="number_format" style="height:30px" class="form-control" id="Cl2_b" name="Cl2_b" placeholder="Enter Cl2" >
							    </div>
							</div>
							<br> 
							<center>EP 3C</center>
							...
							<div class="form-group row my-0">
							    <label for="Cl2_c" class="col-sm-4 col-form-label">Cl2</label>
							    <div class="col-sm-8">
							        <input type="number_format" style="height:30px" class="form-control" id="Cl2_c" name="Cl2_c" placeholder="Enter Cl2" >
							    </div>
							</div>
	...
	<script>
	    function validateForm() {
	        ...
	        var Cl2_b = document.getElementById("Cl2_b").value;

	        var inputs = [SiO2_b, Al2O3_b, Fe2O3_b, CaO_b, MgO_b, SO3_b, K2O_b, Na2O_b, Cl2_b, SiO2_c, Al2O3_c, Fe2O3_c, CaO_c, MgO_c, SO3_c, K2O_c, Na2O_c, Cl2_c];
