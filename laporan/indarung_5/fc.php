...
<body>
    ...
    $sql = "UPDATE fc_5 SET JAM=?, fc90u=?, H2O_fc=?, ASH=?, SHIFT=?, H2O_rc=? WHERE id=?";
    ...
    $stmt->bind_param("ssssssi", $JAM_e, $fc90u_e, $H2Ofc_e, $ASH_e, $SHIFT_e, $H2Orc_e, $id_e);
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM fc_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
}elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM fc_5 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
else {
    $sql_filtered = "SELECT * FROM fc_5 WHERE TANGGAL = CURDATE()";
}
...
?>
		...
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>FINE COAL & RAW COAL Indarung 5</label>
		</center>
	...
	<table class="adminlist mb-3" border="1" width="500" cellpadding="5" >
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        ...
				        $count_query = mysqli_query($conn, "SELECT COUNT(fc90u) AS count_fc90u, COUNT(H2O_fc) AS count_h2ofc, COUNT(ASH) AS count_ASH, COUNT(ADB) AS count_adb, COUNT(H2O_rc) AS count_h2orc FROM fc_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        $count_result = mysqli_fetch_assoc($count_query);
				        foreach ($count_result as $count) {
				            echo "<td>".$count."</td>";
				        }
				    } else {
				        $sql = mysqli_query($conn,"SELECT * FROM fc_5");
			      ...
			      <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $min_query = mysqli_query($conn, "SELECT MIN(fc90u) AS min_fc90u, MIN(H2O_fc) AS min_h2ofc, MIN(ASH) AS min_ASH, MIN(ADB) AS min_adb, MIN(H2O_rc) AS min_h2orc FROM fc_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			     $average_query = mysqli_query($conn, "SELECT AVG(fc90u) AS avg_fc90u, AVG(H2O_fc) AS avg_h2ofc, AVG(ASH) AS avg_ASH, AVG(ADB) AS avg_adb, AVG(H2O_rc) AS avg_h2orc FROM fc_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(fc90u) AS max_fc90u, MAX(H2O_fc) AS max_h2ofc, MAX(ASH) AS max_ASH, MAX(ADB) AS max_adb, MAX(H2O_rc) AS max_h2orc FROM fc_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			   ...
			   <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
				        $akhir = $_GET['akhir'];
				      $sd_query = mysqli_query($conn, "SELECT STDDEV(fc90u) AS sd_fc90u, STDDEV(H2O_fc) AS sd_h2ofc, STDDEV(ASH) AS sd_ASH, STDDEV(ADB) AS sd_adb, STDDEV(H2O_rc) AS sd_h2orc FROM fc_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
		...
		<div>
			 <table class="table table-striped table-bordered table-hover m">
				...
				<tbody>
					    ...
					    $update_query = "UPDATE fc_5 SET ADB=" . (!is_null($adb) ? "'$adb'" : "NULL") . " WHERE id=" . $d['id'];
							...
							$w_ash = '';
					        if ($d['ASH'] > $configs['ACfc5']['max'] ) {
					            $w_ash = 'text-danger';
					        }
					        $w_h2ofc = '';
					        if ($d['H2O_fc'] > $configs['H2Ofc5']['max'] ) {
					            $w_h2ofc = 'text-danger';
					        }
					        $w_90u  = '';
							if ($d['fc90u'] > $configs['fc90u5']['max']) {
							    $w_90u = 'text-danger';
							}
							$w_h2orc = '';
							if ($d['H2O_rc'] > 40) {
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM fc_5 where id ='$id' ";
											...
											?>
												...
												<div class="form-group row my-0">
													...
													<div class="col-sm-8">
														<input type="text" style="height:30px" class="form-control" id="ADB_e" name="SHIFT_e" value="<?= $row['SHIFT']; ?>">
													</div>
												</div>
	            ...
	            <div class="modal-body">
	               <?php
	                    ...
	                    if (isset($_POST['submit'])) {
	                       ...
	                       $sql = "INSERT INTO fc_5 (TANGGAL, JAM, fc90u, H2O_fc, ASH, SHIFT, H2O_rc, waktu, iduser) 
							        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
							$stmt = $conn->prepare($sql);
