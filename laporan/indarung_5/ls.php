...
<body>
    ...
    $sql = "UPDATE ls_5 SET SiO2 = ?, Al2O3 = ?, Fe2O3 = ?, CaO = ?, MgO = ?, SO3 = ?, K2O = ?, Na2O = ?, Cl2 = ?, H2O = ?, PILE = ?, TIANG = ?, JAM = ? WHERE id = ?";
    ...
    mysqli_stmt_bind_param($stmt, "ddddddddddsssi", $SiO2_e, $Al2O3_e, $Fe2O3_e, $CaO_e, $MgO_e, $SO3_e, $K2O_e, $Na2O_e, $Cl2_e, $H2O_e, $PILE_e, $TIANG_e, $JAM_e, $id_e);
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM ls_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
}elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM ls_5 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
} 
else {
    $sql_filtered = "SELECT * FROM ls_5 WHERE TANGGAL = CURDATE()";
}
...
?>
		...
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>LIMESTONE Indarung 5</label>
		</center>
	...
	<table class="adminlist mb-3" border="1" width="500" cellpadding="5" >
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				      ...
				      $count_query = mysqli_query($conn, "SELECT COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(Cl2) AS count_cl2, COUNT(SUM) AS count_sum, COUNT(ALKALI) AS count_alkali, COUNT(H2O) AS count_h2o FROM ls_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM ls_5");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LSF) AS max_lsf, MAX(SIM) AS max_sim, MAX(ALM) AS max_alm, MAX(SiO2) AS max_sio2, MAX(Al2O3) AS max_al2o3, MAX(Fe2O3) AS max_fe2o3, MAX(CaO) AS max_cao, MAX(MgO) AS max_mgo, MAX(SO3) AS max_so3, MAX(K2O) AS max_k2o, MAX(Na2O) AS max_na2o, MAX(Cl2) AS max_cl2, MAX(SUM) AS max_sum, MAX(ALKALI) AS max_alkali, MAX(H2O) AS max_h2o FROM ls_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
					...
					if (isset($_GET['awal']) && isset($_GET['akhir'])) {
					    ...
					    $datain_query = mysqli_query($conn, "SELECT 
					        ...
					        COUNT(IF(Al2O3 > 1.7, 1, NULL)) AS datain_al2o3,
					        COUNT(IF(CaO < 46, 1, NULL)) AS datain_cao
					        FROM ls_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <table class="table table-striped table-bordered table-hover m">
				<thead>
					...
					<tr class="bg-primary text-white">
						...
						<th>Tanggal</th><th>LSF</th> <th>SIM</th> <th>ALM</th><th>SiO2</th> <th>Al2O3</th> <th>Fe2O3</th> <th>CaO</th> <th>MgO</th> <th>SO3</th> <th>K2O</th> <th>Na2O</th> <th>Cl2</th><th>SUM</th><th>Alkali</th><th>H2O</th> <th>PILE</th> <th>TIANG</th><th>JAM SAMPLING</th><th>KETERANGAN</th> <th>Report</th>
				...
				<tbody>
					    ...
					    while ($d = mysqli_fetch_array($q)) {
					       ...
					       $update_query = "UPDATE ls_5 SET LSF=?, SIM=?, ALM=?, SUM=?, ALKALI=? WHERE id=?";
							...
							$w_al2o3 = '';
							if ($d['Al2O3'] < $configs['Al2O3qc']['min']  || $d['Al2O3'] >= $configs['Al2O3qc']['max'] ) {
							    $w_al2o3 = 'text-danger';
							}
							$w_alkali = '';
					        if ($alkali > $configs['ALKALIqc']['max']  ) {
					            $w_alkali = 'text-danger';
					        }
					        $w_sio2 = '';
							if ($d['SiO2'] < $configs['SiO2qc']['min']  || $d['SiO2'] > $configs['SiO2qc']['max'] ) {
							    $w_sio2 = 'text-danger';
							}
							$w_cao = '';
					        if ($d['CaO'] < $configs['CaOqc']['min']  ) {
					            $w_cao = 'text-danger';
					        }
					        $w_h2o = '';
					        if ($d['H2O'] > 6 ) {
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM ls_5 where id ='$id' ";
											...
											?>
												...
												<div class="form-group row my-0">
												    <label for="JAM" class="col-sm-4 col-form-label">JAM SAMPLING</label>
												    <div class="col-sm-8">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					if (isset($_POST['submit'])) {
					    ...
					    $sql = "INSERT INTO ls_5 (TANGGAL, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, Cl2, H2O, PILE, TIANG, JAM, waktu, iduser) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
					    $stmt = mysqli_prepare($conn, $sql);
					    mysqli_stmt_bind_param($stmt, "ssssssssssssssss", $tanggal, $SiO2, $Al2O3, $Fe2O3, $CaO, $MgO, $SO3, $K2O, $Na2O, $Cl2, $H2O, $PILE, $TIANG, $JAM, $waktu, $username);
					...
					$sql = "SELECT * FROM ls_5 ORDER BY TANGGAL DESC";
					$q = mysqli_query($conn, $sql);
					?>
					...
					<div class="container">
						<form method="POST" onsubmit="return validateForm()">
							...
							<div class="form-group row my-0">
							    <label for="JAM" class="col-sm-4 col-form-label">JAM SAMPLING</label>
							    <div class="col-sm-8">
							        <input type="text" style="height:30px" class="form-control" id="JAM" name="JAM" placeholder="Enter JAM SAMPLING" >
							    </div>
							</div>
