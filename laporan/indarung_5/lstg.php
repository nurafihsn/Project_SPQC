...
<body>
	...
	if (isset($_REQUEST['update'])) {
	    ...
	    $sql = "UPDATE lstg_5 SET JAM=?, SiO2=?, Al2O3=?, Fe2O3=?, CaO=?, MgO=?, SO3=?, K2O=?, Na2O=?, Cl2=?, H2O=?, PILE=?, TIANG=?, TONASE=?, CR=?, SYSTEM=? WHERE id=?";
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM lstg_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM lstg_5 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
else {
    $sql_filtered = "SELECT * FROM lstg_5 WHERE TANGGAL = CURDATE()";
}
...
?>
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>LIMESTONE STORAGE Indarung 5</label>
		</center>
	...
	<table class="adminlist mb-3" border="1" width="500" cellpadding="5" >
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
				      $count_query = mysqli_query($conn, "SELECT COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(Cl2) AS count_cl2, COUNT(SUM) AS count_sum, COUNT(ALKALI) AS count_alkali, COUNT(H2O) AS count_h2o FROM lstg_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM lstg_5");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LSF) AS max_lsf, MAX(SIM) AS max_sim, MAX(ALM) AS max_alm, MAX(SiO2) AS max_sio2, MAX(Al2O3) AS max_al2o3, MAX(Fe2O3) AS max_fe2o3, MAX(CaO) AS max_cao, MAX(MgO) AS max_mgo, MAX(SO3) AS max_so3, MAX(K2O) AS max_k2o, MAX(Na2O) AS max_na2o, MAX(Cl2) AS max_cl2, MAX(SUM) AS max_sum, MAX(ALKALI) AS max_alkali, MAX(H2O) AS max_h2o FROM lstg_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <tr bgcolor="YELLOW" style="color:BLACK">
					...
					if (isset($_GET['awal']) && isset($_GET['akhir'])) {
					    ...
					    $datain_query = mysqli_query($conn, "SELECT 
					        ...
					        FROM lstg_5 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <table class="table table-striped table-bordered table-hover">
				<thead>
					...
					<tr class="bg-primary text-white">
						...
						<th>Tanggal</th><th>Jam</th><th>LSF</th> <th>SIM</th> <th>ALM</th><th>SiO2</th> <th>Al2O3</th> <th>Fe2O3</th> <th>CaO</th> <th>MgO</th> <th>SO3</th> <th>K2O</th> <th>Na2O</th> <th>Cl2</th><th>SUM</th><th>Alkali</th><th>H2O</th> <th>PILE</th> <th>TIANG</th> <th>TONASE</th><th>CR</th><th>SYSTEM</th><th>KETERANGAN</th><th>Report</th>
				...
				<tbody>
					    ...
					    while ($d = mysqli_fetch_array($q)) {
					       ...
					       $update_query = "UPDATE lstg_5 SET LSF=?, SIM=?, ALM=?, SUM=?, ALKALI=? WHERE id=?";
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM lstg_5 where id ='$id' order by id";
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                if (isset($_POST['submit'])) {
                    ...
                    $sql = "INSERT INTO lstg_5 (TANGGAL, JAM, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, Cl2, H2O, PILE, TIANG, TONASE, CR, SYSTEM, waktu, iduser) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $stmt = mysqli_prepare($conn, $sql);
                ...
                $sql = "SELECT * FROM lstg_5 WHERE TANGGAL = CURDATE()";
                $q = mysqli_query($conn, $sql);
                ?>
