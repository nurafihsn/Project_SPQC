...
<body>
    ...
    $TYPE_e = isset($_GET['TYPE_e']) && $_GET['TYPE_e'] !== '' ? $_GET['TYPE_e'] : NULL;
    $id_e = isset($_GET['id_e']) && $_GET['id_e'] !== '' ? $_GET['id_e'] : NULL;

    $sql = "UPDATE kf2_3 SET JAM=?, kf90u=?, kf180u=?, H2O=?, SiO2=?, Al2O3=?, Fe2O3=?, CaO=?, MgO=?, SO3=?, K2O=?, Na2O=?, TYPE=? WHERE id=?";
    ...
    mysqli_stmt_bind_param($stmt, "sssssssssssssi", $JAM_e, $kf90u_e, $kf180u_e, $H2O_e, $SiO2_e, $Al2O3_e, $Fe2O3_e, $CaO_e, $MgO_e, $SO3_e, $K2O_e, $Na2O_e, $TYPE_e, $id_e);

    if (mysqli_stmt_execute($stmt)) {
        echo "<script>window.location = 'kf2.php'</script>";
    } 
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM kf2_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM kf2_3 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
else {
    $sql_filtered = "SELECT * FROM kf2_3 WHERE TANGGAL = CURDATE()";
}
...
?>
		...
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>KILN FEED 2 Indarung 2&3</label>
		</center>
	...
	<table class="adminlist mb-3" border="1" width="500" cellpadding="5" >
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
				       $count_query = mysqli_query($conn, "SELECT COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(kf90u) AS count_kf90u, COUNT(kf180u) AS count_kf180u, COUNT(H2O) AS count_h2o, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(SUM) AS count_sum FROM kf2_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM kf2_3");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LSF) AS max_lsf, MAX(SIM) AS max_sim, MAX(ALM) AS max_alm,MAX(kf90u) as max_kf90u,MAX(kf180u) as max_kf180u, MAX(H2O) AS max_h2o, MAX(SiO2) AS max_sio2, MAX(Al2O3) AS max_al2o3, MAX(Fe2O3) AS max_fe2o3, MAX(CaO) AS max_cao, MAX(MgO) AS max_mgo, MAX(SO3) AS max_so3, MAX(K2O) AS max_k2o, MAX(Na2O) AS max_na2o, MAX(SUM) AS max_sum FROM kf2_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
	...
	<div class="container">
	    <div class="row">
	        <div class="col-sm-3">
	            ...
	            <div class="form-group">
	                <a href="exportkf2.php" class="btn btn-primary" > Download </a>
	            </div>
	        </div>        
	    </div>
	</div>
			 ...
			 <table class="table table-striped table-bordered table-hover m">
				...
				<tbody>
					    ...
					    while ($d = mysqli_fetch_array($q)) {
							...
							$sum = null;
							if (!empty($d['SiO2'])) {
							    $sum = $d['SiO2'] + $d['Al2O3'] + $d['Fe2O3'] + ($d['CaO'] * 1.786) + ($d['MgO'] * 2.089) + $d['SO3'] + $d['K2O']  + $d['Na2O'] ;
							}
					        ...
					        $update_query = "UPDATE kf2_3 SET LSF=?, SIM=?, ALM=?, SUM=?, BF=? WHERE id=?";
							$stmt = mysqli_prepare($conn, $update_query);

							mysqli_stmt_bind_param($stmt, "ssssid", $lsf, $sim, $alm, $sum, $bf, $d['id']);
					    ...
					    ?>
					        <tr>
					            ...
					            <td align="center">
					                ...
					                <a href="del-kf2.php?delId=<?php echo $d['id']; ?>" type="button" class="btn btn-danger" style="height:34px" onClick="return confirm('Are you sure to delete this data?');"><i class="fa fa-fw fa-trash"></i> Delete</a>
					            </td>
					        </tr>
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM kf2_3 where id ='$id' ";
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					if (isset($_POST['submit'])) {
					    ...
					    $sql = "INSERT INTO kf2_3 (TANGGAL, JAM, kf90u, kf180u, H2O, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, TYPE, waktu, iduser) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

					    $stmt = mysqli_prepare($conn, $sql);

					    if ($stmt) {

					        mysqli_stmt_bind_param($stmt, "ssssssssssssssss", $tanggal, $JAM, $kf90u, $kf180u, $H2O, $SiO2, $Al2O3, $Fe2O3, $CaO, $MgO, $SO3, $K2O, $Na2O, $TYPE, $waktu, $username);
					...
					$sql = "SELECT * FROM kf2_3 ORDER BY TANGGAL DESC";
					$q = mysqli_query($conn, $sql);
					?>
	...
	<script>
	    function validateForm() {
	        ...
	        var inputs = [kf90u,kf180u,SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, H2O];
