...
<body>
    ...
    $sql = "UPDATE fc_3 SET JAM=?, fc90uk2=?, H2O_fck2=?, ASHk2=?, fc90uk3=?, H2O_fck3=?, ASHk3=?, SHIFT=?, H2O_rc=? WHERE id=?";
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM fc_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM fc_3 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
else {
    $sql_filtered = "SELECT * FROM fc_3 WHERE TANGGAL = CURDATE()";
}
...
?>
		...
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>FINE COAL & RAW COAL Indarung 2&3</label>
		</center>
	...
	<table class="adminlist mb-3" border="1" width="500" cellpadding="5" >
		<tr>
			<th colspan="1" ></th><th colspan="4" class="text-center">2K1</th><th colspan="4" class="text-center">3K2</th><th colspan="1"></th>
		</tr>
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        ...
				        $count_query = mysqli_query($conn, "SELECT COUNT(fc90uk2) AS count_fc90uk2, COUNT(H2O_fck2) AS count_H2O_fck2, COUNT(ASHk2) AS count_ASHk2,COUNT(fc90uk3) AS count_fc90uk3, COUNT(H2O_fck3) AS count_H2O_fck3, COUNT(ASHk3) AS count_ASHk3, COUNT(ADBk2) AS count_ADBk2, COUNT(ADBk3) AS count_ADBk3, COUNT(H2O_rc) AS count_h2orc FROM fc_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        $count_result = mysqli_fetch_assoc($count_query);
				        foreach ($count_result as $count) {
				            echo "<td>".$count."</td>";
				        }
				    } else {
				        $sql = mysqli_query($conn,"SELECT * FROM fc_3");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			     $average_query = mysqli_query($conn, "SELECT AVG(fc90uk2) AS avg_fc90uk2, AVG(H2O_fck2) AS avg_H2O_fck2, AVG(ASHk2) AS avg_ASHk2,AVG(fc90uk3) AS avg_fc90uk3, AVG(H2O_fck3) AS avg_H2O_fck3, AVG(ASHk3) AS avg_ASHk3, AVG(ADBk2) AS avg_ADBk2,AVG(ADBk3) AS avg_ADBk3, AVG(H2O_rc) AS avg_h2orc FROM fc_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			   ...
			   <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
				        $akhir = $_GET['akhir'];
				      $sd_query = mysqli_query($conn, "SELECT STDDEV(fc90uk2) AS sd_fc90uk2, STDDEV(H2O_fck2) AS sd_H2O_fck2, STDDEV(ASHk2) AS sd_ASHk2,STDDEV(fc90uk3) AS sd_fc90uk3, STDDEV(H2O_fck3) AS sd_H2O_fck3, STDDEV(ASHk3) AS sd_ASHk3, STDDEV(ADBk2) AS sd_ADBk2,STDDEV(ADBk3) AS sd_ADBk3, STDDEV(H2O_rc) AS sd_h2orc FROM fc_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <table class="table table-striped table-bordered table-hover m">
				<thead>
					...
					<tr class="text-center" >
			             <th  rowspan="1" ></th><th  rowspan="1" ></th><th colspan="4">FINE COAL 2K1</th><th colspan="4">FINE COAL 3K2</th><th colspan="2">RAW COAL</th> <th colspan="1"></th>
			        </tr>

					<tr class="bg-primary text-white">
						
						<th>Tanggal</th><th>Jam</th><th>90u</th><th>H2O</th><th>ASH</th> <th>ADB</th><th>90u</th><th>H2O</th><th>ASH</th> <th>ADB</th> <th>SHIFT</th> <th>H2O</th> <th>Report</th><th class="text-center">Action</th>
					</tr>
				</thead>
				...
				<tbody>
					    ...
					    $update_query = "UPDATE fc_3 SET ADBk2=" . (!is_null($ADBk2) ? "'$ADBk2'" : "NULL") . ", ADBk3=" . (!is_null($ADBk3) ? "'$ADBk3'" : "NULL") ." WHERE id=" . $d['id'];
							...
							$w_ASHk2 = '';
					        if ($d['ASHk2'] >  $configs['ACfc6']['max'] ) {
					            $w_ASHk2 = 'text-danger';
					        }
					        $w_H2O_fck2 = '';
					        if ($d['H2O_fck2'] >  $configs['H2Ofc6']['max'] ) {
					            $w_H2O_fck2 = 'text-danger';
					        }
					        $w_90uk2  = '';
							if ($d['fc90uk2'] > $configs['fc90u6']['max']) {
							    $w_90uk2 = 'text-danger';
							}
							$w_h2orc = '';
							if ($d['H2O_rc'] > 40) {
							...
							$w_ASHk3 = '';
					        if ($d['ASHk3'] >  $configs['ACfc6']['max'] ) {
					            $w_ASHk3 = 'text-danger';
					        }
					        $w_H2O_fck3 = '';
					        if ($d['H2O_fck3'] >  $configs['H2Ofc6']['max'] ) {
					            $w_H2O_fck3 = 'text-danger';
					        }
					        $w_90uk3  = '';
							if ($d['fc90uk3'] > $configs['fc90u6']['max']) {
							    $w_90uk3 = 'text-danger';
							}
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM fc_3 where id ='$id' ";
											...
											?>
												...
												<div class="form-group row my-0">
													<label for="ASHk2" class="col-sm-4 col-form-label">ASH</label>
													<div class="col-sm-8">
												...
												<div class="form-group row my-0">
													<label for="ASHk3" class="col-sm-4 col-form-label">ASH</label>
													<div class="col-sm-8">
	        ...
	        <div class="modal-content">
	            ...
	            <div class="modal-body">
					...
					if (isset($_POST['submit'])) {
						...
						$sql = "INSERT INTO fc_3 (TANGGAL, JAM, fc90uk2, H2O_fck2, ASHk2, fc90uk3, H2O_fck3, ASHk3, SHIFT, H2O_rc, waktu, iduser) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
						$stmt = mysqli_prepare($conn, $sql);
					...
					<div class="container" >
						<form method="POST" onsubmit="return validateForm()">
							...
							<div class="form-group row my-0">
								<label for="ASHk2" class="col-sm-4 col-form-label">ASH</label>
								<div class="col-sm-8">
									<input type="text" style="height:30px" class="form-control" id="ASHk2" name="ASHk2" placeholder="Enter ASH" >
								</div>
							</div>
							<br>
								<center>3K2</center>
							...
							<div class="form-group row my-0">
								<label for="ASHk3" class="col-sm-4 col-form-label">ASH</label>
								<div class="col-sm-8">
									<input type="text" style="height:30px" class="form-control" id="ASHk3" name="ASHk3" placeholder="Enter ASH" >
								</div>
							</div>
							<br></br>
							<div class="form-group row my-0">
