...
<body>
	...
	if (isset($_REQUEST['update'])) {
		...
		$sql = "UPDATE cr2_3 SET JAM=?, LTW=?, TEMP=?, WARNA=?, FISIK=?, SILO=?, TYPE=?, BERAT_sampel=?, LOLOS_ayakan=?, SiO2=?, Al2O3=?, Fe2O3=?, CaO=?, MgO=?, SO3=?, K2O=?, Na2O=?, FCaO=? WHERE id=?";
		...
		mysqli_stmt_bind_param($stmt,"sssssssdddddddddddi",  $JAM_e, $LTW_e, $TEMP_e,$WARNA_e, $FISIK_e, $SILO_e, $TYPE_e, $BERAT_sampel_e, $LOLOS_ayakan_e, $SiO2_e, $Al2O3_e, $Fe2O3_e, $CaO_e, $MgO_e, $SO3_e, $K2O_e, $Na2O_e, $FCaO_e, $id_e);

		if (mysqli_stmt_execute($stmt)) {
		    echo "<script>window.location = 'cr2.php'</script>";
		} 
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM cr2_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM cr2_3 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
else {
    $sql_filtered = "SELECT * FROM cr2_3 WHERE TANGGAL = CURDATE()";
}
...
?>
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>CLINKER 2 Indarung 2&3</label>
		</center>
	...
	<table class="adminlist mb-3 " border="1" width="500" cellpadding="5"  >
		<thead>
					...
					<tr class="text-center" >
			            <th rowspan="1"></th><th colspan="9">CLINKER</th><th  colspan="10">OKSIDA</th>
			        </tr>

				<tr bgcolor="#8080FF" style="color:#FFFFFF">
			        <th></th>
			        <th>LTW</th><th>TEMP</th> <th>LSF</th><th>SIM</th><th>ALM</th><th>C3S</th><th>C2S</th><th>C3A</th><th>C4AF</th><th>SiO2</th><th>Al2O3</th><th>Fe2O3</th><th>CaO</th><th>MgO</th><th>SO3</th><th>K2O</th><th>Na2O</th><th>FCaO</th><th>SUM</th>
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				      ...
				      $count_query = mysqli_query($conn, "SELECT COUNT(LTW) AS count_LTW, COUNT(TEMP) AS count_TEMP, COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(C3S) AS count_c3s, COUNT(C2S) AS count_c2s, COUNT(C3A) AS count_c3a, COUNT(C4AF) AS count_c4af, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(FCaO) AS count_fcao, COUNT(SUM) AS count_sum FROM cr2_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM cr2_3");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LTW) AS max_LTW, MAX(TEMP) AS max_TEMP, MAX(LSF) AS max_lsf, MAX(SIM) AS max_sim, MAX(ALM) AS max_alm, MAX(C3S) AS max_c3s, MAX(C2S) AS max_c2s, MAX(C3A) AS max_c3a, MAX(C4AF) AS max_c4af, MAX(SiO2) AS max_sio2, MAX(Al2O3) AS max_al2o3, MAX(Fe2O3) AS max_fe2o3, MAX(CaO) AS max_cao, MAX(MgO) AS max_mgo, MAX(SO3) AS max_so3, MAX(K2O) AS max_k2o, MAX(Na2O) AS max_na2o, MAX(FCaO) AS max_fcao, MAX(SUM) AS max_sum  FROM cr2_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				...
				<tr bgcolor="YELLOW" style="color:BLACK">
					...
					if (isset($_GET['awal']) && isset($_GET['akhir'])) {
					   ...
					   $datain_query = mysqli_query($conn, "SELECT 
				            ...
				            FROM cr2_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
	...
	<div class="container">
	    <div class="row">
	        <div class="col-sm-3">
	            ...
	            <div class="form-group">
	                <a href="exportcr2.php" class="btn btn-primary" > Download </a>
	            </div>
	        </div>        
	    </div>
	</div>
		...
		<div>
			 <table class="table table-striped table-bordered table-hover">
				...
				<tbody>
					    ...
					    $update_query = "UPDATE cr2_3 SET LSF=" . (!is_null($lsf) ? "'$lsf'" : "NULL") . ", SIM=" . (!is_null($sim) ? "'$sim'" : "NULL") . ", ALM=" . (!is_null($alm) ? "'$alm'" : "NULL") . ", SUM=" . (!is_null($sum) ? "'$sum'" : "NULL") . ", C3S=" . (!is_null($c3s) ? "'$c3s'" : "NULL") . ", C2S=" . (!is_null($c2s) ? "'$c2s'" : "NULL") . ", C3A=" . (!is_null($c3a) ? "'$c3a'" : "NULL") . ", C4AF=" . (!is_null($c4af) ? "'$c4af'" : "NULL") . ", PERSEN_lolos=" . (!is_null($persen) ? "'$persen'" : "NULL") . " WHERE id=" . $d['id'];
					        ...
					        $w_fcao = '';
					        if ($d['FCaO'] > $configs['FCaOcr6']['max']  ) {
					            $w_fcao = 'text-danger';
					        }
					        $w_c3s = '';
							if ($c3s < $configs['C3Scr6']['min']  || $c3s > $configs['C3Scr6']['max'] ) {
							    $w_c3s = 'text-danger';
							}
							$w_alm = '';
							if ($alm < $configs['ALMcr6']['min']  || $alm > $configs['ALMcr6']['max'] ) {
							    $w_alm = 'text-danger';
							}
					        $w_sim = '';
							if ($sim < $configs['SIMcr6']['min']  || $sim > $configs['SIMcr6']['max'] ) {
							    $w_sim = 'text-danger';
							}
							$w_lsf = '';
							if ($lsf < $configs['LSFcr6']['min']  || $lsf > $configs['LSFcr6']['max'] ) {
							    $w_lsf = 'text-danger';
							}
							$w_ltw = '';
							if ($d['LTW'] < $configs['LTWcr6']['min'] || $d['LTW'] > $configs['LTWcr6']['max']) {
							    $w_ltw = 'text-danger';
							}
							$w_c3a = '';
							if ($c3a < $configs['C3Acr6']['min'] || $c3a > $configs['C3Acr6']['max']) {
							    $w_c3a = 'text-danger';
							}
					    ...
					    ?>
					            ...
					            <td align="center">
					                ...
					                <a href="del-cr2.php?delId=<?php echo $d['id']; ?>" type="button" class="btn btn-danger" style="height:34px" onClick="return confirm('Are you sure to delete this data?');"><i class="fa fa-fw fa-trash"></i> Delete</a>
					            </td>
					        </tr>
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM cr2_3 where id ='$id' order by id";
											...
											?>
												...
												<div class="form-group row my-0">
												 ...
												 <div class="form-group row my-0">
												    ...
												    <div class="col-sm-8">
												        <input type="text" style="height:30px" class="form-control" id="SILO_e" name="SILO_e" value="<?= $row['SILO']; ?>">
												    </div>
												</div>
												 <div class="form-group row my-0">
	            ...
	            <div class="modal-body">
	                 <?php
	                    ...
	                    if (isset($_POST['submit'])) {
	                       ...
	                       $sql = "INSERT INTO cr2_3 (TANGGAL, JAM, LTW, TEMP, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, FCaO, WARNA, FISIK, SILO,TYPE , BERAT_sampel, LOLOS_ayakan, waktu, iduser) 
        						VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
					    $stmt = mysqli_prepare($conn, $sql);

					    if ($stmt) {
					       mysqli_stmt_bind_param($stmt, "ssssdddddddddssssddss", $tanggal, $JAM, $LTW, $TEMP, $SiO2, $Al2O3, $Fe2O3, $CaO, $MgO, $SO3, $K2O, $Na2O, $FCaO, $WARNA, $FISIK, $SILO, $TYPE , $BERAT_sampel, $LOLOS_ayakan, $waktu, $username);
	                    ...
	                    $sql = "SELECT * FROM cr2_3 WHERE TANGGAL = CURDATE()";
	                    $q = mysqli_query($conn, $sql);
	                ?>
					...
					<div class="container">
						<form method="POST" onsubmit="return validateForm()">
							...
							<div class="form-group row my-0">
							    <label for="SILO" class="col-sm-4 col-form-label">SILO</label>
							    <div class="col-sm-8">
							        <input type="text" style="height:30px" class="form-control" id="SILO" name="SILO" placeholder="Enter SILO" >
							    </div>
							</div>
							<div class="form-group row my-0">
							    <label for="TYPE" class="col-sm-4 col-form-label">TYPE</label>
	...
	<script>
	    function validateForm() {
	        ...
	        var inputs = [LTW, TEMP,SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, FCaO, LOLOS_ayakan, BERAT_sampel];
