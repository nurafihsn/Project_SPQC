...
<body>
	...
	if (isset($_REQUEST['update'])) {
	    ...
	    $PILE2_e = isset($_GET['PILE2_e']) && $_GET['PILE2_e'] !== '' ? $_GET['PILE2_e'] : NULL;
	    $TIANG2_e = isset($_GET['TIANG2_e']) && $_GET['TIANG2_e'] !== '' ? $_GET['TIANG2_e'] : NULL;
	    $PILE3_e = isset($_GET['PILE3_e']) && $_GET['PILE3_e'] !== '' ? $_GET['PILE3_e'] : NULL;
	    $TIANG3_e = isset($_GET['TIANG3_e']) && $_GET['TIANG3_e'] !== '' ? $_GET['TIANG3_e'] : NULL;
	    $TONASE_e = isset($_GET['TONASE_e']) && $_GET['TONASE_e'] !== '' ? $_GET['TONASE_e'] : NULL;
	    $CR_e = isset($_GET['CR_e']) && $_GET['CR_e'] !== '' ? $_GET['CR_e'] : NULL;
	    $id_e = isset($_GET['id_e']) && $_GET['id_e'] !== '' ? $_GET['id_e'] : NULL;

	    $sql = "UPDATE lstg_3 SET JAM=?, SiO2=?, Al2O3=?, Fe2O3=?, CaO=?, MgO=?, SO3=?, K2O=?, Na2O=?, H2O=?, PILE2=?, TIANG2=?,  PILE3=?, TIANG3=?, TONASE=?, CR=? WHERE id=?";
	    ...
	    mysqli_stmt_bind_param($stmt, "ssssssssssssssssi", $JAM_e, $SiO2_e, $Al2O3_e, $Fe2O3_e, $CaO_e, $MgO_e, $SO3_e, $K2O_e, $Na2O_e, $H2O_e, $PILE2_e, $TIANG2_e, $PILE3_e, $TIANG3_e, $TONASE_e, $CR_e, $id_e);

	    if (mysqli_stmt_execute($stmt)) {
	        echo "<script>window.location = 'lstg.php'</script>";
	    }
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM lstg_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM lstg_3 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
else {
    $sql_filtered = "SELECT * FROM lstg_3 WHERE TANGGAL = CURDATE()";
}
...
?>
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>LIMESTONE STORAGE Indarung 2&3</label>
		</center>
	...
	<table class="adminlist mb-3" border="1" width="500" cellpadding="5" >
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
				      $count_query = mysqli_query($conn, "SELECT COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(SUM) AS count_sum, COUNT(H2O) AS count_h2o FROM lstg_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM lstg_3");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LSF) AS max_lsf, MAX(SIM) AS max_sim, MAX(ALM) AS max_alm, MAX(SiO2) AS max_sio2, MAX(Al2O3) AS max_al2o3, MAX(Fe2O3) AS max_fe2o3, MAX(CaO) AS max_cao, MAX(MgO) AS max_mgo, MAX(SO3) AS max_so3, MAX(K2O) AS max_k2o, MAX(Na2O) AS max_na2o, MAX(SUM) AS max_sum, MAX(H2O) AS max_h2o FROM lstg_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <tr bgcolor="YELLOW" style="color:BLACK">
					...
					if (isset($_GET['awal']) && isset($_GET['akhir'])) {
					    ...
					    $datain_query = mysqli_query($conn, "SELECT 
					        COUNT(IF(SiO2 > 10, 1, NULL)) AS datain_sio2,
					        COUNT(IF(Al2O3 > 1.6, 1, NULL)) AS datain_al2o3,
					        COUNT(IF(CaO < 46, 1, NULL)) AS datain_cao
					        FROM lstg_3 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
	...
	<div class="container">
	    <div class="row">
	        <div class="col-sm-3">
	            ...
	            <div class="form-group">
	                <a href="exportlstg.php" class="btn btn-primary" > Download </a>
	            </div>
	        </div>        
	    </div>
	</div>
			 ...
			 <table class="table table-striped table-bordered table-hover">
				<thead>
					...
					<tr class="text-center" >
			            <th  rowspan="1" ></th><th  rowspan="1" ></th><th colspan="3">MODULUS</th><th colspan="8">OKSIDA</th> <th colspan="1"></th> <th colspan="3"></th><th colspan="2">IND 2</th><th colspan="2">IND 3</th><th colspan="1"></th>
			        </tr>

					<tr class="bg-primary text-white">
						
						<th>Tanggal</th><th>Jam</th><th>LSF</th> <th>SIM</th> <th>ALM</th><th>SiO2</th> <th>Al2O3</th> <th>Fe2O3</th> <th>CaO</th> <th>MgO</th> <th>SO3</th> <th>K2O</th> <th>Na2O</th> <th>SUM</th><th>H2O</th> <th>TONASE</th><th>CR</th><th>PILE</th> <th>TIANG</th><th>PILE</th> <th>TIANG</th><th>KETERANGAN</th><th>Report</th>
				...
				<tbody>
					    ...
					    while ($d = mysqli_fetch_array($q)) {
					       ...
					       $update_query = "UPDATE lstg_3 SET LSF=?, SIM=?, ALM=?, SUM=? WHERE id=?";
							$stmt = mysqli_prepare($conn, $update_query);

							mysqli_stmt_bind_param($stmt, "ssssi", $lsf, $sim, $alm, $sum, $d['id']);
					        ...
					        $w_h2o = '';
					        if ($d['H2O'] > $configs['H2Ols']['max'] ) {
					            $w_h2o = 'text-danger';
					        }
							$w_cao = '';
					        if ($d['CaO'] < $configs['CaOls']['min']  ) {
					            $w_cao = 'text-danger';
					        }
					        $w_al2o3 = '';
					        if ($d['Al2O3'] > $configs['Al2O3ls']['max'] || $d['Al2O3'] < $configs['Al2O3ls']['min'] ) {
					            $w_al2o3 = 'text-danger';
					        }
					       $w_sio2 = '';
					        if ($d['SiO2'] > $configs['SiO2ls']['max'] || $d['SiO2'] < $configs['SiO2ls']['min'] ) {
					            $w_sio2 = 'text-danger';
					        }
					    ?>
					            ...
					            <td><?php echo $d['CR']; ?></td>
					            <td><?php echo $d['PILE2']; ?></td>
					            <td><?php echo $d['TIANG2']; ?></td>
					            <td><?php echo $d['PILE3']; ?></td>
					            <td><?php echo $d['TIANG3']; ?></td>
					            ...
					            <td align="center">
					                ...
					                <a href="del-lstg.php?delId=<?php echo $d['id']; ?>" type="button" class="btn btn-danger" style="height:34px" onClick="return confirm('Are you sure to delete this data?');"><i class="fa fa-fw fa-trash"></i> Delete</a>
					            </td>
					        </tr>
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM lstg_3 where id ='$id' order by id";
											...
											?>
												...
												<div class="form-group row my-0">
												    ...
												    <div class="col-sm-8">
												        <input type="text" style="height:30px" class="form-control" id="TONASE_e" name="TONASE_e" value="<?= $row['TONASE']; ?>">
												    </div>
												</div>
												<div class="form-group row my-0">
												    <label for="CR" class="col-sm-4 col-form-label">CR</label>
												    <div class="col-sm-8">
												        <select name="CR_e" id="CR_e" style="height:30px" class="form-control">
												            <option value=""></option> 
												            <option value="II" <?= ($row['CR'] == 'II') ? 'selected' : '' ?>>II</option>
												            <option value="IIIA" <?= ($row['CR'] == 'IIIA') ? 'selected' : '' ?>>IIIA</option>
												            <option value="IIIB" <?= ($row['CR'] == 'IIIB') ? 'selected' : '' ?>>IIIB</option>
												            <option value="VI" <?= ($row['CR'] == 'VI') ? 'selected' : '' ?>>VI</option>
												            <option value="MS1" <?= ($row['CR'] == 'MS1') ? 'selected' : '' ?>>MS1</option>
												        </select>
												    </div>
												</div>
												<div class="form-group row my-0">
												    <label for="PILE2" class="col-sm-4 col-form-label">PILE IND2</label>
												    <div class="col-sm-8">
												        <input type="text" style="height:30px" class="form-control" id="PILE2_e" name="PILE2_e" value="<?= $row['PILE2']; ?>">
												    </div>
												</div>
												<div class="form-group row my-0">
												    <label for="TIANG2" class="col-sm-4 col-form-label">TIANG IND2</label>
												    <div class="col-sm-8">
												        <input type="text" style="height:30px" class="form-control" id="TIANG2_e" name="TIANG2_e" value="<?= $row['TIANG2']; ?>">
												    </div>
												</div>
												<div class="form-group row my-0">
												    <label for="PILE3" class="col-sm-4 col-form-label">PILE IND3</label>
												    <div class="col-sm-8">
												        <input type="text" style="height:30px" class="form-control" id="PILE3_e" name="PILE3_e" value="<?= $row['PILE3']; ?>">
												    </div>
												</div>
												<div class="form-group row my-0">
												    <label for="TIANG3" class="col-sm-4 col-form-label">TIANG IND3</label>
												    <div class="col-sm-8">
												        <input type="text" style="height:30px" class="form-control" id="TIANG3_e" name="TIANG3_e" value="<?= $row['TIANG3']; ?>">
												    </div>
												</div>
	...
	$PILE2 = !empty($_POST['PILE2']) ? $_POST['PILE2'] : NULL;
	$TIANG2 = !empty($_POST['TIANG2']) ? $_POST['TIANG2'] : NULL;
	$PILE3 = !empty($_POST['PILE3']) ? $_POST['PILE3'] : NULL;
	$TIANG3 = !empty($_POST['TIANG3']) ? $_POST['TIANG3'] : NULL;
	$TONASE = !empty($_POST['TONASE']) ? $_POST['TONASE'] : NULL;
...
<div class="modal fade" id="addData" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                if (isset($_POST['submit'])) {
                    ...
                    $sql = "INSERT INTO lstg_3 (TANGGAL, JAM, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, H2O, PILE2, TIANG2,PILE3, TIANG3, TONASE, CR,  waktu, iduser) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $stmt = mysqli_prepare($conn, $sql);
                    mysqli_stmt_bind_param($stmt, "sssssssssssssssssss", $tanggal, $JAM, $SiO2, $Al2O3, $Fe2O3, $CaO, $MgO, $SO3, $K2O, $Na2O, $H2O, $PILE2, $TIANG2, $PILE3, $TIANG3, $TONASE, $CR, $waktu, $username);
                ...
                $sql = "SELECT * FROM lstg_3 WHERE TANGGAL = CURDATE()";
                $q = mysqli_query($conn, $sql);
                ?>
					...
					<div class="container">
						<form method="POST" onsubmit="return validateForm()">
							...
							<div class="form-group row my-0">
							    <label for="TONASE" class="col-sm-4 col-form-label">TONASE</label>
							    <div class="col-sm-8">
							        <input type="text" style="height:30px" class="form-control" id="TONASE" name="TONASE" placeholder="Enter TONASE" >
							    </div>
							</div>
							<div class="form-group row my-0">
							    <label for="CR" class="col-sm-4 col-form-label">CR</label>
							    <div class="col-sm-8">
							        <select name="CR" id="CR" style="height:30px" class="form-control">
							            <option value=""></option> 
							...
							<div class="form-group row my-0">
							    <label for="PILE2" class="col-sm-4 col-form-label">PILE IND2</label>
							    <div class="col-sm-8">
							        <input type="text" style="height:30px" class="form-control" id="PILE2" name="PILE2" placeholder="Enter PILE IND2" >
							    </div>
							</div>
							<div class="form-group row my-0">
							    <label for="TIANG2" class="col-sm-4 col-form-label">TIANG IND2</label>
							    <div class="col-sm-8">
							        <input type="text" style="height:30px" class="form-control" id="TIANG2" name="TIANG2" placeholder="Enter TIANG IND2" >
							    </div>
							</div>
							<div class="form-group row my-0">
							    <label for="PILE3" class="col-sm-4 col-form-label">PILE IND3</label>
							    <div class="col-sm-8">
							        <input type="text" style="height:30px" class="form-control" id="PILE3" name="PILE3" placeholder="Enter PILE IND3" >
							    </div>
							</div>
							<div class="form-group row my-0">
							    <label for="TIANG3" class="col-sm-4 col-form-label">TIANG IND3</label>
							    <div class="col-sm-8">
							        <input type="text" style="height:30px" class="form-control" id="TIANG3" name="TIANG3" placeholder="Enter TIANG IND3" >
							    </div>
							</div>
	...
	<script>
	    function validateForm() {
	        ...
	        var inputs = [SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, H2O];
