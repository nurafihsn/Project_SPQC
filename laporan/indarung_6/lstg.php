...
<body>
	...
	if (isset($_REQUEST['update'])) {
	    ...
	    $PILE_e = isset($_GET['PILE_e']) && $_GET['PILE_e'] !== '' ? $_GET['PILE_e'] : NULL;
	    $TIANG_e = isset($_GET['TIANG_e']) && $_GET['TIANG_e'] !== '' ? $_GET['TIANG_e'] : NULL;
	    $TONASE_e = isset($_GET['TONASE_e']) && $_GET['TONASE_e'] !== '' ? $_GET['TONASE_e'] : NULL;
	    $CR_e = isset($_GET['CR_e']) && $_GET['CR_e'] !== '' ? $_GET['CR_e'] : NULL;
	    $SYSTEM_e = isset($_GET['SYSTEM_e']) && $_GET['SYSTEM_e'] !== '' ? $_GET['SYSTEM_e'] : NULL;
	    $id_e = isset($_GET['id_e']) && $_GET['id_e'] !== '' ? $_GET['id_e'] : NULL;

	    $sql = "UPDATE lstg_6 SET JAM=?, SiO2=?, Al2O3=?, Fe2O3=?, CaO=?, MgO=?, SO3=?, K2O=?, Na2O=?, Cl2=?, H2O=?, PILE=?, TIANG=?, TONASE=?, CR=?, SYSTEM=? WHERE id=?";
	    ...
	    mysqli_stmt_bind_param($stmt, "ssssssssssssssssi", $JAM_e, $SiO2_e, $Al2O3_e, $Fe2O3_e, $CaO_e, $MgO_e, $SO3_e, $K2O_e, $Na2O_e, $Cl2_e, $H2O_e, $PILE_e, $TIANG_e, $TONASE_e, $CR_e, $SYSTEM_e, $id_e);
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM lstg_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} 
elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM lstg_6 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}

 else {
    $sql_filtered = "SELECT * FROM lstg_6 WHERE TANGGAL = CURDATE()";
}
...
?>
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>LIMESTONE STORAGE Indarung 6</label>
		</center>
	...
	<table class="adminlist mb-3" border="1" width="500" cellpadding="5" >
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
				      $count_query = mysqli_query($conn, "SELECT COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(Cl2) AS count_cl2, COUNT(SUM) AS count_sum, COUNT(ALKALI) AS count_alkali, COUNT(H2O) AS count_h2o FROM lstg_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM lstg_6");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LSF) AS max_lsf, MAX(SIM) AS max_sim, MAX(ALM) AS max_alm, MAX(SiO2) AS max_sio2, MAX(Al2O3) AS max_al2o3, MAX(Fe2O3) AS max_fe2o3, MAX(CaO) AS max_cao, MAX(MgO) AS max_mgo, MAX(SO3) AS max_so3, MAX(K2O) AS max_k2o, MAX(Na2O) AS max_na2o, MAX(Cl2) AS max_cl2, MAX(SUM) AS max_sum, MAX(ALKALI) AS max_alkali, MAX(H2O) AS max_h2o FROM lstg_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
					...
					if (isset($_GET['awal']) && isset($_GET['akhir'])) {
					    ...
					    $akhir = $_GET['akhir'];
					     ...
					     $datain_query = mysqli_query($conn, "SELECT 
					        ...
					        COUNT(IF(Al2O3 > 2, 1, NULL)) AS datain_al2o3,
					        COUNT(IF(CaO < 46, 1, NULL)) AS datain_cao
					        FROM lstg_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <table class="table table-striped table-bordered table-hover">
				<thead>
					...
					<tr class="text-center" >
			            <th  rowspan="1" ></th><th  rowspan="1" ></th><th colspan="3">MODULUS</th><th colspan="10">OKSIDA</th> <th colspan="1"></th> <th colspan="6">LIMESTONE STORAGE</th><th colspan="1"></th>
			        </tr>

					<tr class="bg-primary text-white">
						
						<th>Tanggal</th><th>Jam</th><th>LSF</th> <th>SIM</th> <th>ALM</th><th>SiO2</th> <th>Al2O3</th> <th>Fe2O3</th> <th>CaO</th> <th>MgO</th> <th>SO3</th> <th>K2O</th> <th>Na2O</th> <th>Cl2</th><th>SUM</th><th>Alkali</th><th>H2O</th> <th>PILE</th> <th>TIANG</th> <th>TONASE</th><th>CR</th><th>SYSTEM</th><th>Report</th>
				...
				<tbody>
					    ...
					    while ($d = mysqli_fetch_array($q)) {
					       ...
					       $update_query = "UPDATE lstg_6 SET LSF=?, SIM=?, ALM=?, SUM=?, ALKALI=? WHERE id=?";
							...
							$w_alkali = '';
					        if ($alkali > $configs['ALKALIls']['max'] ) {
					    ...
					    ?>
					            ...
					            <td><?php echo $d['PILE']; ?></td>
					            <td><?php echo $d['TIANG']; ?></td>
					            <td><?php echo $d['TONASE']; ?></td>
					            <td><?php echo $d['CR']; ?></td>
					            <td><?php echo $d['SYSTEM']; ?></td>
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM lstg_6 where id ='$id' order by id";
											...
											?>
												...
												<div class="form-group row my-0">
												    <label for="PILE" class="col-sm-4 col-form-label">PILE</label>
												    <div class="col-sm-8">
												        <input type="text" style="height:30px" class="form-control" id="PILE_e" name="PILE_e" value="<?= $row['PILE']; ?>">
												    </div>
												</div>
												<div class="form-group row my-0">
												    <label for="TIANG" class="col-sm-4 col-form-label">TIANG</label>
												    <div class="col-sm-8">
												        <input type="text" style="height:30px" class="form-control" id="TIANG_e" name="TIANG_e" value="<?= $row['TIANG']; ?>">
												    </div>
												</div>
												<div class="form-group row my-0">
												...
												<div class="form-group row my-0">
												    ...
												    <div class="col-sm-8">
												        <select name="SYSTEM_e" id="SYSTEM_e" style="height:30px" class="form-control">
												            <option value=""></option> 
												            <option value="CHEVRON" <?= ($row['SYSTEM'] == 'CHEVRON') ? 'selected' : '' ?>>CHEVRON</option>
												            <option value="CONE" <?= ($row['SYSTEM'] == 'CONE') ? 'selected' : '' ?>>CONE</option>
	...
	$PILE = !empty($_POST['PILE']) ? $_POST['PILE'] : NULL;
	$TIANG = !empty($_POST['TIANG']) ? $_POST['TIANG'] : NULL;
	$TONASE = !empty($_POST['TONASE']) ? $_POST['TONASE'] : NULL;
	$CR = !empty($_POST['CR']) ? $_POST['CR'] : NULL;
	$SYSTEM = !empty($_POST['SYSTEM']) ? $_POST['SYSTEM'] : NULL;
...
<div class="modal fade" id="addData" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                if (isset($_POST['submit'])) {
                    ...
                    $sql = "INSERT INTO lstg_6 (TANGGAL, JAM, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, Cl2, H2O, PILE, TIANG, TONASE, CR, SYSTEM, waktu, iduser) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $stmt = mysqli_prepare($conn, $sql);
                    mysqli_stmt_bind_param($stmt, "sssssssssssssssssss", $tanggal, $JAM, $SiO2, $Al2O3, $Fe2O3, $CaO, $MgO, $SO3, $K2O, $Na2O, $Cl2, $H2O, $PILE, $TIANG, $TONASE, $CR, $SYSTEM, $waktu, $username);
                ...
                $sql = "SELECT * FROM lstg_6 WHERE TANGGAL = CURDATE()";
                $q = mysqli_query($conn, $sql);
                ?>
					...
					<div class="container">
						<form method="POST" onsubmit="return validateForm()">
							...
							<div class="form-group row my-0">
							    <label for="PILE" class="col-sm-4 col-form-label">PILE</label>
							    <div class="col-sm-8">
							        <input type="text" style="height:30px" class="form-control" id="PILE" name="PILE" placeholder="Enter PILE" >
							    </div>
							</div>
							<div class="form-group row my-0">
							    <label for="TIANG" class="col-sm-4 col-form-label">TIANG</label>
							    <div class="col-sm-8">
							        <input type="text" style="height:30px" class="form-control" id="TIANG" name="TIANG" placeholder="Enter TIANG" >
							    </div>
							</div>
							<div class="form-group row my-0">
							    <label for="TONASE" class="col-sm-4 col-form-label">TONASE</label>
							...
							<div class="form-group row my-0">
							    <label for="SYSTEM" class="col-sm-4 col-form-label">SYSTEM</label>
							    <div class="col-sm-8">
							        <select name="SYSTEM" id="SYSTEM" style="height:30px" class="form-control">
			                            <option value="" ></option>
	...
	<script>
	    function validateForm() {
	        ...
	        var inputs = [SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, Cl2, H2O];
