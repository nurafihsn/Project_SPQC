...
<body>
	...
	if (isset($_REQUEST['update'])) {
		...
		$sql = "UPDATE cr_6 SET JAM=?, LTW=?, TEMP=?, WARNA=?, FISIK=?, SILO=?, TUJUAN=?, BERAT_sampel=?, LOLOS_ayakan=?, SiO2=?, Al2O3=?, Fe2O3=?, CaO=?, MgO=?, SO3=?, K2O=?, Na2O=?, Cl2=?, FCaO=? WHERE id=?";
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM cr_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM cr_6 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
} 
else {
    $sql_filtered = "SELECT * FROM cr_6 WHERE TANGGAL = CURDATE()";
}
...
?>
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>CLINKER Indarung 6</label>
		</center>
		...
		<thead>
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				      ...
				      $count_query = mysqli_query($conn, "SELECT COUNT(LTW) AS count_LTW, COUNT(TEMP) AS count_TEMP, COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(C3S) AS count_c3s, COUNT(C2S) AS count_c2s, COUNT(C3A) AS count_c3a, COUNT(C4AF) AS count_c4af, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(Cl2) AS count_cl2, COUNT(FCaO) AS count_fcao, COUNT(SUM) AS count_sum, COUNT(ALKALI) AS count_alkali, COUNT(LP) AS count_lp, COUNT(CI) AS count_ci, COUNT(GI) AS count_gi, COUNT(BF) AS count_bf, COUNT(BI) AS count_bi, COUNT(ASR) AS count_asr, COUNT(DoS) AS count_dos, COUNT(EstBZT) AS count_est FROM cr_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM cr_6");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LTW) AS max_LTW, MAX(TEMP) AS max_TEMP, MAX(LSF) AS max_lsf, MAX(SIM) AS max_sim, MAX(ALM) AS max_alm, MAX(C3S) AS max_c3s, MAX(C2S) AS max_c2s, MAX(C3A) AS max_c3a, MAX(C4AF) AS max_c4af, MAX(SiO2) AS max_sio2, MAX(Al2O3) AS max_al2o3, MAX(Fe2O3) AS max_fe2o3, MAX(CaO) AS max_cao, MAX(MgO) AS max_mgo, MAX(SO3) AS max_so3, MAX(K2O) AS max_k2o, MAX(Na2O) AS max_na2o, MAX(Cl2) AS max_cl2, MAX(FCaO) AS max_fcao, MAX(SUM) AS max_sum, MAX(ALKALI) AS max_alkali, MAX(LP) AS max_lp, MAX(CI) AS max_ci, MAX(GI) AS max_gi, MAX(BF) AS max_bf, MAX(BI) AS max_bi, MAX(ASR) AS max_asr, MAX(DoS) AS max_dos, MAX(EstBZT) AS max_est FROM cr_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				...
				<tr bgcolor="YELLOW" style="color:BLACK">
					...
					if (isset($_GET['awal']) && isset($_GET['akhir'])) {
					   ...
					   $datain_query = mysqli_query($conn, "SELECT 
				            ...
				            FROM cr_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <table class="table table-striped table-bordered table-hover">
				...
				<tbody>
					    ...
					    $update_query = "UPDATE cr_6 SET LSF=" . (!is_null($lsf) ? "'$lsf'" : "NULL") . ", SIM=" . (!is_null($sim) ? "'$sim'" : "NULL") . ", ALM=" . (!is_null($alm) ? "'$alm'" : "NULL") . ", SUM=" . (!is_null($sum) ? "'$sum'" : "NULL") . ", ALKALI=" . (!is_null($alkali) ? "'$alkali'" : "NULL") . ", C3S=" . (!is_null($c3s) ? "'$c3s'" : "NULL") . ", C2S=" . (!is_null($c2s) ? "'$c2s'" : "NULL") . ", C3A=" . (!is_null($c3a) ? "'$c3a'" : "NULL") . ", C4AF=" . (!is_null($c4af) ? "'$c4af'" : "NULL") . ", LP=" . (!is_null($lp) ? "'$lp'" : "NULL") . ", CI=" . (!is_null($ci) ? "'$ci'" : "NULL") . ", GI=" . (!is_null($gi) ? "'$gi'" : "NULL") . ", BF=" . (!is_null($bf) ? "'$bf'" : "NULL") . ", BI=" . (!is_null($bi) ? "'$bi'" : "NULL") . ", ASR=" . (!is_null($asr) ? "'$asr'" : "NULL") . ", DoS=" . (!is_null($dos) ? "'$dos'" : "NULL") . ", EstBZT=" . (!is_null($est) ? "'$est'" : "NULL") . ", PERSEN_lolos=" . (!is_null($persen) ? "'$persen'" : "NULL") . " WHERE id=" . $d['id'];
							...
							$w_alkali = '';
					        if ($alkali > $configs['ALKALIcr6']['max']  ) {
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM cr_6 where id ='$id' order by id";
												 ...
												 <div class="form-group row my-0">
												    ...
												    <div class="col-sm-8">
												        <select name="TUJUAN_e" id="TUJUAN_e" style="height:30px" class="form-control">
												            ...
												            <option value="IND4" <?= ($row['TUJUAN'] == 'IND4') ? 'selected' : '' ?>>IND4</option>
	            ...
	            <div class="modal-body">
	                <?php
	                    ...
	                    if (isset($_POST['submit'])) {
	                       ...
	                       $sql = "INSERT INTO cr_6 (TANGGAL, JAM, LTW, TEMP, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, Cl2, FCaO, WARNA, FISIK, SILO, TUJUAN, BERAT_sampel, LOLOS_ayakan, waktu, iduser) 
        						VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
					    $stmt = mysqli_prepare($conn, $sql);
							...
							<div class="form-group row my-0">
							    ...
							    <div class="col-sm-8">
							        <select name="TUJUAN" id="TUJUAN" style="height:30px" class="form-control">
			                            ...
			                            <option value="IND4">IND 4</option>
