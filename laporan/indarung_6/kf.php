...
<body>
    ...
    $sql = "UPDATE kf_6 SET JAM=?, kf90u=?, kf180u=?, H2O=?, SiO2=?, Al2O3=?, Fe2O3=?, CaO=?, MgO=?, SO3=?, K2O=?, Na2O=?, Cl2=?, arah_BHF=?, LEVEL_cf=?, FEED_kiln=?, FEED_fc=? WHERE id=?";
...
if (isset($_GET['awal']) && isset($_GET['akhir'])) {
    $awal = $_GET['awal'];
    $akhir = $_GET['akhir'];
    $sql_filtered = "SELECT * FROM kf_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'";
} elseif (isset($_GET['kemarin'])) {
    $sql_filtered = "SELECT * FROM kf_6 WHERE TANGGAL = CURDATE() - INTERVAL 1 DAY";
}
else {
    $sql_filtered = "SELECT * FROM kf_6 WHERE TANGGAL = CURDATE()";
}
...
?>
		...
		<center style="font-family: Arial, sans-serif; font-size: 24px; font-weight: bold; color: #333333; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">
		    <label>KILN FEED Indarung 6</label>
		</center>
	...
	<table class="adminlist mb-3" border="1" width="500" cellpadding="5" >
			    <tr bgcolor="#8080FF" style="color:#FFFFFF">
			     ...
			     <tr bgcolor="YELLOW" style="color:BLACK">
				    ...
				    if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
				       $count_query = mysqli_query($conn, "SELECT COUNT(LSF) AS count_lsf, COUNT(SIM) AS count_sim, COUNT(ALM) AS count_alm, COUNT(kf90u) AS count_kf90u, COUNT(kf180u) AS count_kf180u, COUNT(H2O) AS count_h2o, COUNT(SiO2) AS count_sio2, COUNT(Al2O3) AS count_al2o3, COUNT(Fe2O3) AS count_fe2o3, COUNT(CaO) AS count_cao, COUNT(MgO) AS count_mgo, COUNT(SO3) AS count_so3, COUNT(K2O) AS count_k2o, COUNT(Na2O) AS count_na2o, COUNT(Cl2) AS count_cl2, COUNT(SUM) AS count_sum, COUNT(ALKALI) AS count_alkali FROM kf_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
				        ...
				        $sql = mysqli_query($conn,"SELECT * FROM kf_6");
			    ...
			    <tr bgcolor="YELLOW" style="color:BLACK">
			        ...
			        if(isset($_GET['awal']) && isset($_GET['akhir'])){
				        $awal = $_GET['awal'];
			        	$akhir = $_GET['akhir'];
			        $max_query = mysqli_query($conn, "SELECT MAX(LSF) AS max_lsf, MAX(SIM) AS max_sim, MAX(ALM) AS max_alm,MAX(kf90u) as max_kf90u,MAX(kf180u) as max_kf180u, MAX(H2O) AS max_h2o, MAX(SiO2) AS max_sio2, MAX(Al2O3) AS max_al2o3, MAX(Fe2O3) AS max_fe2o3, MAX(CaO) AS max_cao, MAX(MgO) AS max_mgo, MAX(SO3) AS max_so3, MAX(K2O) AS max_k2o, MAX(Na2O) AS max_na2o, MAX(Cl2) AS max_cl2, MAX(SUM) AS max_sum, MAX(ALKALI) AS max_alkali FROM kf_6 WHERE TANGGAL BETWEEN '$awal' AND '$akhir'");
			 ...
			 <table class="table table-striped table-bordered table-hover m">
				...
				<tbody>
					    ...
					    while ($d = mysqli_fetch_array($q)) {
					        ...
					        $update_query = "UPDATE kf_6 SET LSF=?, SIM=?, ALM=?, SUM=?, ALKALI=?, BF=?, TON_emisi=? WHERE id=?";
					        ...
					        $w_180u = '';
					        if ($d['kf180u'] > $configs['kf180u6']['max'] ) {
					            $w_180u = 'text-danger';
					        }
					        $w_90u = '';
							if ($d['kf90u'] > $configs['kf90u6']['max'] ) {
							    $w_90u = 'text-danger';
							}
							$w_h2o = '';
							if ($d['H2O'] < $configs['H2Okf6']['min'] || $d['H2O'] > $configs['H2Okf6']['max']) {
							    $w_h2o = 'text-danger';
							}
							$w_alm = '';
							if ($alm < $configs['ALMkf6']['min'] || $alm > $configs['ALMkf6']['max']) {
							    $w_alm = 'text-danger';
							}
					        $w_sim = '';
							if ($sim < $configs['SIMkf6']['min'] || $sim > $configs['SIMkf6']['max']) {
							    $w_sim = 'text-danger';
							}
							$w_lsf = '';
							if ($lsf < $configs['LSFkf6']['min'] || $lsf > $configs['LSFkf6']['max']) {
							    $w_lsf = 'text-danger';
							}
					            ...
					            <div class="modal-dialog">
									...
									<div class="modal-body">
										<form role="form" action="" method="get">
											...
											<?php
											$id = $d['id'];
											$sql = "SELECT * FROM kf_6 where id ='$id' ";
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					if (isset($_POST['submit'])) {
					    ...
					    $sql = "INSERT INTO kf_6 (TANGGAL, JAM, kf90u, kf180u, H2O, SiO2, Al2O3, Fe2O3, CaO, MgO, SO3, K2O, Na2O, Cl2, arah_BHF, LEVEL_cf, FEED_kiln, FEED_fc, waktu, iduser) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
					...
					$sql = "SELECT * FROM kf_6 ORDER BY TANGGAL DESC";
					$q = mysqli_query($conn, $sql);
					?>
